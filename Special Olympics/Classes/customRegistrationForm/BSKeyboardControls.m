    ...
    if (index > 0) {
        ...
        PreviousField: {
            if ([self.activeTextField isKindOfClass:[UITextField class]]) {
                if (((UITextField*)self.activeTextField).isEnabled == NO) {
                    previous -= 1;
                    self.activeTextField = [self.textFields objectAtIndex:previous];
                    goto PreviousField;
                }
...
-(void)setShowSegment:(BOOL)showSegment1
{
    self.segmentedPreviousNext.hidden = !showSegment1;
}
...
- (IBAction)segmentedControlPreviousNextChangedValue:(id)sender
{
    self.isVirtualKeyPadPressed = YES;
